...
async function getBattingLeaderboard() {
  try {
    // Use Prisma's raw query capabilities instead
    const battingPerformances = await prisma.$queryRaw`
      SELECT 
        ...
        cp."role" as "playerRole",
        ct."id" as "teamId",
        ct."name" as "teamName"
      FROM "CricketBatting" cb
      JOIN "CricketPlayer" cp ON cb."playerId" = cp."id"
      JOIN "CricketTeam" ct ON cp."teamId" = ct."id"
    `;
    
    // Group by player and aggregate stats
    const playerStats = new Map();
    
    for (const performance of battingPerformances as any[]) {
      ...
      if (!playerStats.has(playerId)) {
        playerStats.set(playerId, {
          player: {
            ...
            role: performance.playerRole,
            team: {
              id: performance.teamId,
    ...
    console.error("Error in getBattingLeaderboard:", error);
    return [];
  }
}
...
async function getBowlingLeaderboard() {
  try {
    ...
    for (const performance of bowlingPerformances as any[]) {
      const playerId = performance.playerId;
      ...
      const stats = playerStats.get(playerId);
      stats.innings += 1;
      stats.overs += Number(performance.overs);
    ...
    console.error("Error in getBowlingLeaderboard:", error);
    return [];
  }
}